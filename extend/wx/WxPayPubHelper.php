...
// 		curl_setopt($ch,CURLOPT_SSLKEY, WxPayConf_pub::SSLKEY_PATH);
		...
		curl_setopt($ch, CURLOPT_CAINFO, SSLCA_PATH); // CA根证书（用来验证的网站证书是否是CA颁布）
...
{	
	function __construct() 
	{
		...
		//设置curl超时时间
		$this->curl_timeout = WX_CURL_TIMEOUT;
	}
...
class NativeCall_pub extends Wxpay_server_pub{
	...
	function createXml()
	{
		if($this->returnParameters["return_code"] == "SUCCESS"){
		   	$this->returnParameters["appid"] = WX_APPID;//公众账号ID
		   	$this->returnParameters["mch_id"] = WX_MCHID;//商户号
...
class Redpack_pub extends Wxpay_client_pub
{
	...
	function createXml()
	{
		try
		{
			...
			$this->parameters["wxappid"] = WX_APPID;//公众账号ID
			$this->parameters["mch_id"] = WX_MCHID;//商户号
